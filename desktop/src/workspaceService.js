...
export async function selectWorkspace() {
    if (!electronAPI?.selectWorkspace) {
        throw new Error(
            "Workspace selection is only available in the Electron desktop app."
        );
    }

    const result =
        await electronAPI.selectWorkspace();
...
export async function openWorkspaceInVSCode(
    ...
    if (!electronAPI?.openVSCode) {
        throw new Error(
            "VS Code integration is only available in the Electron desktop app."
        );
    }
