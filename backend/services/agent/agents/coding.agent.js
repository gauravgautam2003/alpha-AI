...
async function runWorkspaceCodingAgent(llm, state) {
    ...
    const messages = [
        new SystemMessage(`
You are Alpha AI's coding agent with access to the user's selected VS Code workspace.
Active Workspace Directory: ${state.workspacePath || "Current Project"}

Use workspace tools when they are needed to inspect, edit, validate, or report on the code.

Work only inside the user's selected workspace.

Read relevant files before editing.

Make focused fixes.

Do not delete files or directories unless the user explicitly asks.

Run only safe validation commands when useful.

After tool use, give a concise summary of:
- files changed
- what was fixed
- verification result
        `),
...
export const codingAgent = async (state) => {
    try {
         ...
         * Workspace coding
         *
         * workspacePath is only metadata from the user's
         * Windows desktop.
         *
         * It must NOT be resolved or accessed inside Docker.
         */
        if (state.workspacePath) {
            const data =
                await runWorkspaceCodingAgent(
                    llm,
                    state
                );
...
Return ONLY the classification keyword in uppercase.

No punctuation or markdown.
...
You MUST output STRICT VALID JSON matching this exact structure:
            ...
            try {
                ...
                if (
                    ...
                    return {
                        ...
                        aiResponse:
                            "✨ **Project code generated successfully.** Explore and run the generated files in your project workspace.",
                ...
                console.warn(
                    "Project JSON extraction fallback to direct markdown response"
                );
            }
        }
...
Clearly identify underlying issues, inefficiencies, or edge cases.

2. Production Code:
Provide clean, idiomatic, fully working code blocks with proper syntax highlighting.

3. Security & Scalability:
Adhere to OWASP security guidelines, error handling, and optimal complexity.

4. Step-by-Step Explanation:
Walk through why this solution works and how to verify it.
...
${state.prompt}
        ...
        return {
            ...
            aiResponse:
                error?.data?.message ||
                error?.message ||
                "❌ Coding agent encountered an error. Please try again.",
